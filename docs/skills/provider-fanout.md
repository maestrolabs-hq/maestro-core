...
| Component | Purpose |
| --- | --- |
| `SKILL.md` | The equivalence mapping (verified tool names cross-checked against `docs/providers/*.md`) and three ready mcpScript templates: query-all, index-all, status-all. Index-all and status-all each pair the mcpScript with a companion shell block for the two CLI-only providers (CodeGraph, Graphify). |
...
- **CodeGraph and Graphify indexing is CLI-only, not a workaround.**
  CodeGraph's MCP server exposes no indexing tool at all — a background
  watcher keeps its index current once `codegraph init` has run once.
  Graphify's MCP server is read-only by design; building or updating its
  graph is a CLI-only operation (`graphify extract` / `graphify global
  add`). Both facts come from the provider docs, not from a limitation this
  skill introduces.
- **Semantica and Docling report real `n/a` semantics.** Semantica has no
  repository indexer — it is fed incrementally by supplied content, not a
  repo path, so "index Semantica" has no equivalent operation. Docling holds
  no repository index at all; its cache and status calls are always scoped
  to one already-converted document, never global.
